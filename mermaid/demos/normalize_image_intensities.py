...
def quantile_to_cdf(qf,nr_of_bins):
    ...
    bins = np.linspace(start=min_qf,stop=max_qf,num=nr_of_bins)
    cdf = []
...
def compute_quantile_function(vals,nr_of_quantiles):
    perc = np.linspace(start=0,stop=100,num=nr_of_quantiles)
...
def histogram_match(imsrc,target_cdf,target_bins,remove_background=True,background_value=0):
    ...
    im3 = np.interp(im2, cdftgt, target_bins)
...
#    avg_quant,perc = remove_average_quantile_outliers(avg_quant_orig,perc_orig,perc_removal=0)
    avg_quant = avg_quant_orig
    perc = perc_orig
    #res = dict()
