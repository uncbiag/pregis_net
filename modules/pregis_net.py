...
class PregisNet(nn.Module):
    ...
    def calculate_evaluation_loss(self, moving, target, normal_mask, disp_field):
        ...
        recons = self.recons.clone()
        normal_indices = (normal_mask > 0.5)
        recons[normal_indices] = target[normal_indices]
        loss_from_mermaid = self.mermaid_net.calculate_evaluation_loss(moving, recons, normal_mask, disp_field)
        loss_dict = {
    ...
    def calculate_loss(self, moving, target, normal_mask):
        ...
        recons = self.recons.clone()
        # since we have mask, keep normal unchanged
        normal_indices = (normal_mask > 0.5)
        recons[normal_indices] = target[normal_indices]
        loss_from_mermaid = self.mermaid_net.calculate_loss(moving, recons, normal_mask)
        loss_dict = {
            'recons_loss': loss_from_recons['recons_loss'],
            'mermaid_all_loss': loss_from_mermaid['mermaid_all_loss'],
            'mermaid_sim_loss': loss_from_mermaid['mermaid_sim_loss'],
    ...
    def forward(self, moving_image, target_image):
        self.mermaid_net(moving_image, target_image)
