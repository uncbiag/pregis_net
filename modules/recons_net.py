...
class ReconsNet(nn.Module):
    def __init__(self, model_config, network_mode):
        ...
        self.fc21 = nn.Linear(1024, 60)
        self.fc22 = nn.Linear(1024, 60)

        self.fc3 = nn.Linear(60, 1024)
        self.fc4 = nn.Linear(1024, 12*12*128)
        ...
        self.encoder_conv11 = ConBnRelDp(1, 16, kernel_size=3, stride=1, dim=dim, activate_unit='leaky_relu',
        ...
        self.encoder_conv4 = ConBnRelDp(32, 64, kernel_size=3, stride=1, dim=dim, activate_unit='leaky_relu',
                                        same_padding=True, use_bn=use_bn, use_dp=use_dp)
        self.encoder_conv5 = ConBnRelDp(64, 128, kernel_size=3, stride=1, dim=dim, activate_unit='leaky_relu',
                                        same_padding=True, use_bn=use_bn, use_dp=use_dp)
        self.encoder_conv6 = ConBnRelDp(128, 128, kernel_size=3, stride=1, dim=dim, activate_unit='leaky_relu',
                                        same_padding=True, use_bn=use_bn, use_dp=use_dp)

        self.decoder_conv1 = ConBnRelDp(128, 128, kernel_size=3, stride=1, dim=dim, activate_unit='leaky_relu',
                                        same_padding=True, use_bn=use_bn, use_dp=use_dp)
        self.decoder_conv2 = ConBnRelDp(128, 64, kernel_size=3, stride=1, dim=dim, activate_unit='leaky_relu',
                                        same_padding=True, use_bn=use_bn, use_dp=use_dp)
    ...
    def encode(self, x1, x2):
        ...
        x = self.encoder_conv6(x).view(-1, np.prod(self.last_conv_sz[2:]) * 128)
    ...
    def decode(self, z):
        ...
        x = self.relu(self.fc4(x)).view(-1, 128, *self.last_conv_sz[2:])
    ...
    def calculate_vae_loss(self, input_image, target_image, current_epoch):
        ...
        all_vae_loss = self.KLD_weight * kld_loss + self.recons_weight * recons_loss_11
    ...
    def forward(self, input_image, target_image, mode='train'):
        ...
        z = self.reparameterize(self.mu, self.log_var, mode)
        # z = self.encode(input_image, target_image)
        self.recons_image = self.decode(z)
        self.diff_image = input_image - self.recons_image.detach()
