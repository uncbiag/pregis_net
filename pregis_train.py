...
class TrainPregis:
    ...
    def __setup__(self):
        if self.is_continue:
            ...
            time = "{:04d}{:02d}{:02d}-{:02d}{:02d}{:02d}".format(now.year,now.month,now.day,now.hour,now.minute,now.second)
            ...
            print("Writing {} to {}".format(self.network_config_file, os.path.join(self.network_folder, 'network_config.json')))
        ...
        while current_epoch < n_epochs:
            ...
            for i, images in enumerate(self.train_data_loader, 0):
                ...
                if (i + 1) % summary_batch_period == 0:  # print summary every k batches
                    ...
                    images_to_show = [ct_image.cpu(), cb_image.cpu(), self.model.warped_image.detach().cpu()]
                    labels_to_show = [ct_sblabel.cpu(), ct_sdlabel.cpu(), cb_sblabel.cpu(), cb_sdlabel.cpu(),
                                      self.model.warped_labels.detach().cpu()[:, [0], ...],
                                      self.model.warped_labels.detach().cpu()[:, [1], ...]]
