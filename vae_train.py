...
def train_model(model, train_data_loader, validate_data_loader, optimizer, scheduler,
    ...
    while current_epoch < n_epochs:
        ...
        for i, (moving_image, _) in enumerate(train_data_loader, 0):
            model.train()
            optimizer.zero_grad()
            ...
            model(moving_image)
            loss_dict = model.calculate_vae_loss(moving_image)
            all_loss = loss_dict['vae_all_loss']
            all_loss.backward()
            ...
            if (i + 1) % summary_batch_period == 0:  # print summary every k batches
                print('====>{:0d}, {:0d}, loss:{:.6f}, recons_loss:{:.6f}, kld_loss:{:.6f}'.format(
                    current_epoch + 1,
                    global_step,
                ...
                images_to_show.append(model.recons_image.detach())
        ...
        if current_epoch % validate_epoch_period == 0:  # validate every k epochs
            with torch.no_grad():
                ...
                for j, (moving_image, _) in enumerate(validate_data_loader, 0):
                    moving_image = moving_image.cuda()
                    model(moving_image)
                    loss_dict = model.calculate_vae_loss(moving_image)
                ...
                print('=EVAL>{:0d}, {:0d}, loss:{:.6f}, recons_loss:{:.6f}, kld_loss:{:.6f}'.format(
                    current_epoch + 1,
                    global_step,
                    eval_loss_dict['vae_all_loss'] / val_iters_per_epoch,
                    eval_loss_dict['vae_recons_loss'] / val_iters_per_epoch,
                    eval_loss_dict['vae_kld_loss'] / val_iters_per_epoch)
                )
                for loss_key in eval_loss_dict:
...
def train_network():
    ...
    model = create_model(model_config, model_name='vae_net')
    optimizer, scheduler = create_optimizer(train_config, model)

    use_TV_loss = model_config['pregis_net']['recons_net']['use_TV_loss']
    kld_weight = model_config['pregis_net']['recons_net']['KLD_weight']
    tv_weight = model_config['pregis_net']['recons_net']['TV_weight']

    # criterion = create_loss(train_config)
    if not is_continue:
        my_name = "model_{}_kld{}_tv{}_useTV{}_lr{}".format(
            my_time,
            kld_weight,
            tv_weight,
            use_TV_loss,
        ...
        model_folder = os.path.join(os.path.dirname(__file__), 'tmp_models', 'vae_net', my_name)
        os.system('mkdir -p ' + model_folder)
        os.system('cp ' + network_config_file + ' ' + model_folder)
        log_folder = os.path.join(os.path.dirname(__file__), 'logs', 'vae_net', my_name)
